//! Keyward Passbook — the consumer credential manager (Phase A).
//!
//! Where `keyward-vault` is the minimal store the *broker* uses, Passbook is the
...
//!   composing the [`keyward_crypto`] shared kernel with the 2SKD twist.
...
//! the public API is a flat `keyward_passbook::{Entry, seal, open, watchtower, …}`.
//!
