...
//!
//! Speaks the native-messaging wire protocol on stdio: each message is a
//! little-endian `u32` length followed by that many bytes of UTF-8 JSON. The
//! browser extension is the only caller (pinned by extension id in the host
//! manifest's `allowed_origins`), so arbitrary web pages cannot reach the vault
//! — unlike a localhost HTTP server, which any page could probe.
//!
...
/// is exhaustively unit-tested. `now` is the unix time for TOTP computation.
fn handle_request(req: &Value, entries: &[Entry], now: u64) -> Value {
    match req.get("type").and_then(Value::as_str) {
        Some("ping") => json!({ "ok": true, "version": env!("CARGO_PKG_VERSION") }),
...
pub fn run(entries: &[Entry]) -> io::Result<()> {
    ...
    while let Some(frame) = read_frame(&mut r)? {
        let reply = match serde_json::from_slice::<Value>(&frame) {
            Ok(req) => handle_request(&req, entries, crate::now_unix()),
...
mod tests {
    ...
    fn list_matches_origin_and_hides_secrets() {
        ...
        let resp = handle_request(&req, &sample(), 0);
    ...
    fn get_returns_secret_and_totp_code() {
        ...
        let resp = handle_request(&req, &sample(), 1_700_000_000);
    ...
    fn get_missing_is_an_error_not_a_panic() {
        let resp = handle_request(&json!({ "type": "get", "id": "nope" }), &sample(), 0);
        assert_eq!(resp["error"], "not found");
    }

    #[test]
    fn ping_reports_version() {
        let resp = handle_request(&json!({ "type": "ping" }), &sample(), 0);
    ...
    fn unknown_request_is_rejected() {
        let resp = handle_request(&json!({ "type": "wat" }), &sample(), 0);
