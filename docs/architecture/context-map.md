...
Keyward is two **bounded contexts** over one **shared kernel**. Each context is a
...
│                                    │             │                                    │
│  Domain core:  keyward-passbook    │             │  Domain:  keyward-vault,           │
│    domain · sealing · watchtower · │             │           keyward-broker,          │
│    sharing · totp · ports          │             │           keyward-mint             │
│                                    │             │                                    │
│  Adapters:                         │             │  Adapters / supporting:            │
│    passbook-cli  (file repo, CLI,  │             │    keyward-mcp   (MCP server)      │
│                   native bridge)   │             │    keyward-cli   (broker CLI)      │
│    passbook-wasm (browser)         │             │    keyward-profiles (external      │
...
`keyward-passbook`, organized into `domain` (entities + value objects), `sealing`
...
A **supporting context** for the Passbook context. `keyward-sync` is the domain
(the `SyncStore` port + `MemoryStore`/`FileStore` adapters + optimistic-concurrency
rule); `keyward-sync-server` is a tiny HTTP adapter exposing it. The server stores
...
is `keyward-passbook`'s `SealedVault` bytes, but to Sync it is just bytes
...
The AI-native broker: `keyward-vault` (its store), `keyward-broker` (capabilities,
origin-binding, propose-not-commit, audit), `keyward-mint` (short-lived tokens),
surfaced via `keyward-mcp` and `keyward-cli`. `keyward-profiles` is a **supporting
generic subdomain** — external, pluggable provider config.

## Relationships (DDD patterns)

- **Shared Kernel** — `keyward-crypto`. Both contexts depend on the exact same
...
`keyward-broker` is the domain core of the developer-wedge context — capabilities,
origin-binding, propose-not-commit, the risk-tiered policy, and the hash-chained
audit trail. Its *driven* dependencies are inverted behind `keyward-broker::ports`,
mirroring Passbook:
...
they are traits in `keyward-mint` (`Minter`, `Executor`, `HttpClient`, …). The
broker selects the `Primitive::Minted` outcome; the executing host (`keyward-mcp`,
`keyward-cli`) supplies the concrete minter/executor adapter. This keeps the broker
core free of provider HTTP and signing code (the anti-corruption boundary to
`keyward-mint` and `keyward-profiles`).
