...
//!
//! Gated on the `KEYWARD_TEST_PG` env var (a libpq URL); skipped otherwise, so the
...
fn pool_or_skip() -> Option<Pool> {
    ...
    POOL.get_or_init(|| match std::env::var("KEYWARD_TEST_PG") {
        Ok(url) if !url.is_empty() => Some(connect(&url, 6).expect("connect + migrate")),
        _ => {
            eprintln!("skipping: set KEYWARD_TEST_PG to a libpq URL to run the Postgres contract");
            None
        }
    })
    .clone()
