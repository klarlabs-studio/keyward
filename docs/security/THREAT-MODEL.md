# Keyward — Threat Model & Security Posture
...
> review** — it is *not* a substitute for one. Keyward must not hold real
> credentials in production until independently audited.

## 1. What Keyward is (asset & purpose)

Keyward lets an AI agent *act* with a credential without the credential entering
...
```
[ model / agent ]  ──MCP (stdio/JSON-RPC)──▶  [ keyward-mcp process ]  ──▶ [ subprocess / HTTP / STS ]
...
### Broker decision core (`keyward-broker`)
...
### Secret handling (`keyward-vault`, broker state)
...
### Minting (`keyward-mint`: github / exchange / aws)
...
### Execution — HTTP (`keyward-mint::exec`)
- **Confused deputy:** origin-bound; the executor only performs the vetted op.
  **Mitigated.**

### Execution — subprocess (`keyward-mint::run`, `run_command`)
...
- **Information disclosure via /proc, ps, child inheritance:** env injection is
  *hygiene, not a boundary* (ADR-0002). **Partially mitigated** by: injecting via
  ...
  `KEYWARD_TRUST=untrusted`, `run_command` is **refused** unless
  `KEYWARD_ISOLATION` is set — the safe posture is a config gate, not advice.
...
1. The **host and OS user** running `keyward-mcp` are trusted (secrets are
   decrypted in this process). Keyward defends the *model/agent* boundary, not a
   compromised host.
2. **`KEYWARD_MASTER`** and the token-endpoint / role config are trusted inputs.
3. TLS to STS / provider APIs is intact (rustls).
4. For untrusted-content-driven autonomy, `KEYWARD_ISOLATION` is set to a real
...
| R4 | Audit log not signed (FS-write attacker can forge) | Medium | **Fixed (v1.10.0):** optional HMAC-signed chain (`KEYWARD_AUDIT_KEY`) |
...
3. ✅ **Done (v1.10.0):** the audit chain can be **HMAC-signed** via
   `KEYWARD_AUDIT_KEY` — forgery requires the key, not just FS write. (Shipping to
...
6. ✅ **Done (v1.8.0):** `KEYWARD_TRUST=untrusted` refuses `run_command` when
...
| T3 | `KEYWARD_MASTER` in env (readable via /proc) **and inherited by every `run_command` child** | Master read from `KEYWARD_MASTER_FILE`; the runner **`env_clear()`s** and re-adds only a minimal baseline + injected creds — the broker's env never reaches children |
...
| T5 | Write access to `$KEYWARD_PROFILES` = command-binding bypass | **Group/world-writable profile files are rejected** at load |
...
not exploitable here since Keyward only *signs*, never validates) — now updated to
